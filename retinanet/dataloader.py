...
class CocoDataset(Dataset):
    ...
    def __init__(self, annotation_rt, image_rt ,transform=None):
        
        self.ann_rt = annotation_rt
        self.img_rt = image_rt
        self.data_path = self.img_rt
        self.transform = transform

        if isinstance(annotation_rt, dict):
            self.coco = COCO()
            self.coco.dataset = annotation_rt
            self.coco.createIndex()
        else:
            self.coco = COCO(annotation_rt)       
    ...
    def __getitem__(self, idx):
        ...
        annot, bboxes, labels = self.load_annotations(idx)
        sample = {'img': img, 'annot': annot, 'bboxes': bboxes, 'labels': labels}
        if self.transform:
        ...
        """if isinstance(idx, slice):
            ...
            return self.transform({
                ...
                "annot": self.load_annotations(idx)["annot"],
                "bboxes": self.load_annotations(idx)["bboxes"],
                "labels": self.load_annotations(idx)["labels"]
            })"""

    def load_image(self, image_index):
        image_info = self.coco.loadImgs(self.image_ids[image_index])[0]
        path       = os.path.join(self.img_rt, image_info['file_name'])
    ...
    def load_annotations(self, image_index):
        ...
        for idx, a in enumerate(coco_annotations):
            ...
            label = self.coco.getCatIds().index(a['category_id'])
            labels.append(label)
        ...
        return {"annot":annotations,"bboxes": bboxes,"labels": labels}
    ...
    def split_by_imgids(self, imgids):
        ...
        return CocoDataset(coco_format, self.data_path, self.transform)
